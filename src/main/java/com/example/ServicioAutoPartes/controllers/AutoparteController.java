...
public class AutoparteController {
    ...
    @GetMapping
    public ResponseEntity<List<Autoparte>> getAllAutopartes() {
        try {
            List<Autoparte> autopartes = autoparteRepository.findAll();
            if (autopartes.isEmpty()) {
                return new ResponseEntity<>(HttpStatus.NO_CONTENT);
            }
            return new ResponseEntity<>(autopartes, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Autoparte> getAutoparteById(@PathVariable("id") Long id) {
        Optional<Autoparte> autoparteData = autoparteRepository.findById(id);
        return autoparteData.map(autoparte -> new ResponseEntity<>(autoparte, HttpStatus.OK))
                .orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    @GetMapping("/codigo/{codigo}")
    public ResponseEntity<Autoparte> getAutoparteByCodigo(@PathVariable("codigo") String codigo) {
    ...
    public ResponseEntity<List<Autoparte>> getAutopartesByModelo(@PathVariable("modeloId") Long modeloId) {
        try {
    ...
    public ResponseEntity<List<Autoparte>> getAutopartesByPieza(@PathVariable("piezaId") Long piezaId) {
        try {
